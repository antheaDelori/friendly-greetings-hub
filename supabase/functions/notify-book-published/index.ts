...
Deno.serve(async (req) => {
  ...
  const { book_id, book_slug, book_titolo } = await req.json();

  if (!book_id || !book_slug || !book_titolo) {
  ...
  const errors: string[] = [];
  let sent = 0;
  const bccAdmin = await shouldBccAdmin(supabase, "libro_pubblicato", false);
