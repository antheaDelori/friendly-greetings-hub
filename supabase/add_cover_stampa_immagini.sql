...
COMMENT ON COLUMN books.cover_aletta_sx_modo    IS 'testo | immagine — se immagine, cover_aletta_sx_img_url sostituisce interamente l''aletta posteriore (niente foto autore/testo auto)';
COMMENT ON COLUMN books.cover_aletta_dx_modo    IS 'testo | immagine — se immagine, cover_aletta_dx_img_url sostituisce interamente l''aletta anteriore';
COMMENT ON COLUMN books.cover_quarta_img_url    IS 'URL immagine custom per il retro, usata quando cover_quarta_modo = immagine';
COMMENT ON COLUMN books.cover_aletta_sx_img_url IS 'URL immagine custom per l''aletta posteriore, usata quando cover_aletta_sx_modo = immagine';
COMMENT ON COLUMN books.cover_aletta_dx_img_url IS 'URL immagine custom per l''aletta anteriore, usata quando cover_aletta_dx_modo = immagine';
