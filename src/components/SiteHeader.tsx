...
export function SiteHeader() {
  const linkBase =
    "text-[11px] font-mono tracking-[0.18em] uppercase text-bone/70 hover:text-cyan transition-colors";
    ...
    <header className="sticky top-0 z-40 border-b border-cyan/15 bg-void/70 backdrop-blur-xl">
      ...
      <div className="border-b border-cyan/10 bg-deep/40">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-10 h-7 flex items-center justify-between font-mono text-[10px] tracking-widest text-cyan/60">
          <div className="flex items-center gap-4">
            <span className="flex items-center gap-1.5"><span className="h-1.5 w-1.5 bg-cyan rounded-full animate-pulse" />SYS:ONLINE</span>
      ...
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-6 px-4 sm:px-6 lg:px-10">
        ...
        <nav className="hidden md:flex items-center gap-7">
          <Link to="/" className={linkBase} activeProps={{ className: "text-cyan text-glow-cyan" }} activeOptions={{ exact: true }}>
            Home
          </Link>
          <Link to="/catalogo" className={linkBase} activeProps={{ className: "text-cyan text-glow-cyan" }}>
            Catalogo
          </Link>
          <Link to="/community" className={linkBase} activeProps={{ className: "text-cyan text-glow-cyan" }}>
            Community
          </Link>
          <Link to="/regolamento" className={linkBase} activeProps={{ className: "text-cyan text-glow-cyan" }}>
            Regole
          </Link>
          <Link to="/donazioni" className={linkBase} activeProps={{ className: "text-cyan text-glow-cyan" }}>
            Sostieni
          </Link>
        </nav>
